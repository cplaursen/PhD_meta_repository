...
proof -
  ...
  have K1_measure: "kernel_measure K\<^sub>1 \<omega> = M"
    apply (rule measure_eqI)
     apply (simp add: K\<^sub>1_def emeasure_transition_kernel)
    using K1 assms(6) by fastforce
  ...
  have int_eq: "(\<integral>\<^sup>+ x. emeasure (return M x \<star> N) A' \<partial>M) = (emeasure (M \<star> N) A')" if "A' \<in> sets N" for A'
    ...
    using assms(4,5) that apply blast
        apply (simp add: assms(4,5) prob_space.finite_measure prob_space_return sets_eq_imp_space_eq)
       apply (simp add: prob_space.finite_measure)
      apply (simp add: assms(4,5) sets_eq_imp_space_eq)
     apply (simp add: assms(4) sets_eq_imp_space_eq)
    apply (subst nn_integral_return)
      apply (simp add: assms(4,5) sets_eq_imp_space_eq)
    using that apply measurable
    apply (metis assms(1,2,4,5) convolution_emeasure prob_space.finite_measure sets_eq_imp_space_eq that)
    done
  show ?thesis
  apply (subst kernel_comp_kernel)
