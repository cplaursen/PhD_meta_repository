...
locale transition_kernel =
  ...
  assumes
    sets_target_measurable [measurable]:
      "\<And>A'. A' \<in> sets M' \<Longrightarrow> (\<lambda>\<omega>. \<kappa> \<omega> A') \<in> M \<rightarrow>\<^sub>M borel" and
    space_source_measure: "\<And>\<omega>. \<omega> \<in> space M \<Longrightarrow>
...
lemma kernel_measurable_pi_system:
  assumes measurable_P[measurable]: "\<And>A'. A' \<in> P \<Longrightarrow> (\<lambda>\<omega>. \<kappa> \<omega> A') \<in> M \<rightarrow>\<^sub>M borel" and
    ...
    measure_space: "\<And>\<omega>. \<omega> \<in> space M \<Longrightarrow> measure_space (space M') (sets M') (\<kappa> \<omega>)" and
    finite: "\<And>\<omega>. \<omega> \<in> space M \<Longrightarrow> \<kappa> \<omega> (space M') < \<infinity>"
...
proof
  ...
  proof -
    ...
    next
      case (compl A)
      then have diff: "\<kappa> \<omega> (space M' - A) = \<kappa> \<omega> (space M') - \<kappa> \<omega> A" if "\<omega> \<in> space M" for \<omega>
        using measure_space[OF that] sorry (* Need the finite assumption here *)
      then show ?case
        apply (subst measurable_cong[OF diff])
         apply blast
        using compl P(2) by measurable
    next
      case (union A)
      then have sum: "\<kappa> \<omega> (\<Union>i. A i) = (\<Sum>i. \<kappa> \<omega> (A i))" if "\<omega> \<in> space M" for \<omega>
        using measure_space[OF that] unfolding measure_space_def countably_additive_def
        by (metis assms(4) sets.countable_nat_UN)
      then show ?case
        apply (subst measurable_cong[OF sum])
        apply blast
        using union by measurable
    qed
  qed
...
lemma kernel_transition_kernel [simp]:
  "transition_kernel (kernel_source K) (kernel_target K) (kernel K)"
  by (transfer, auto)
...
lemma kernel_empty [simp]: "kernel K \<omega> {} = 0"
  apply (cases "\<omega> \<in> space (kernel_source K)")
   apply transfer
  using transition_kernel.positive[unfolded positive_def] apply fastforce
  apply simp
  done
...
lemma source_kernel_of [simp, measurable]: "kernel_source (kernel_of M M' \<kappa>) = M"
  by (transfer, auto)

lemma target_kernel_of [simp, measurable]: "kernel_target (kernel_of M M' \<kappa>) = M'"
  by (transfer, auto)
...
lemma measurable_kernel_measure[measurable]: "(f \<in> (kernel_measure K \<omega>) \<rightarrow>\<^sub>M M) = (f \<in> (kernel_target K) \<rightarrow>\<^sub>M M)"
  by (auto simp: measurable_cong_sets[of "kernel_measure K \<omega>" "kernel_target K" M M])

lemma kernel_measure_kernel_of [simp]:
...
lemma finite_kernelI:
  ...
  apply (intro_locales)
   apply (meson assms finite_measure_def sigma_finite_kernel.intro)
  apply (simp add: assms finite_kernel_axioms.intro)
  done
...
lemma stochastic_kernelI [intro]:
...
lemma stochastic_kernel_altI [intro]:
...
lemma stochastic_kernel_kernel_ofI [intro!]:
  assumes "\<And>\<omega>. \<omega> \<in> space M \<Longrightarrow> \<kappa> \<omega> (space M') = 1"
      and "transition_kernel M M' \<kappa>"
    shows "stochastic_kernel (kernel_of M M' \<kappa>)"
...
lemma (in stochastic_kernel) kernel_measure_emeasure_finite [simp]:
  "emeasure (kernel_measure K \<omega>) (space (kernel_target K)) \<noteq> \<infinity>"
  by (simp add: finite_kernel_axioms finite_kernel_finite)
